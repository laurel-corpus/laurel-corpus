...
scanned and most poems have been given a metre. I built it as the material underneath
...
them here, since a number written in two places tends to be wrong in one of them.
...
metre is a pattern of stressed and unstressed syllables that a poem keeps to more or less consistently.
Iambic pentameter, by far the most common metre in English, is a line of ten syllables that alternate
...
metre exactly. The places where a line departs from its pattern are seldom errors. More often than not,
the departure is where the poet is doing something deliberate. For this reason every line in the corpus
is recorded twice, once as the metre asks for it and once as the words themselves require.

```xml
<l n="7" real="+-x+-x-+-x" met="-+-+-+-+-+">Making a famine where abundance lies,</l>
```

The `met` attribute records what the metre asks for. Sonnet 1 is in iambic pentameter, so the pattern
...
does not settle, which the metre is free to take either way.
...
The metre wants the line to open softly and place the stress on the second syllable. The word will not
...
the word itself. Those syllables are left for the metre to take as it likes, and they are marked as open
rather than guessed at.

Keeping both rows is, in my view, what makes the corpus useful. A corpus that stores only the metre
...
To count which poems are in which metre:
...
Each entry has the form `[metre, confidence, source, foot, feet]`. The `source` field says how the metre
...
To find where a poem departs from its metre, which is the question the corpus was built to answer:
...
not a citation, because translations differ and so do editions. Laurel follows CTS, the scheme that
...
taken from and links to it. The Gutenberg header, footer, and licence text have been removed, and
...
to the 1920s, which means that it favours poets who are read over poets who are representative. Editions,
...
corpus claims to know. They cover how a metre is determined and what counts as evidence, how accurate
...
The largest of those limits has recently been narrowed. A metre used to be named whenever the words fell
...
of that length. A metre is now named only where more than half of a poem's lines are of a length that the
metre can actually produce. This is a test that the confidence score alone could not make, since the
poems it catches were scoring well. Well over a thousand poems, roughly one in thirteen, moved from a
wrong answer to no answer as a result. What remains is that a poem with no metre is still not
distinguished from a poem whose metre could not be settled, and a small number of pages that are not
...
rhyme lettering, the metre determinations, the canonical references, the identifiers, and the audio
